...
|Sig extension |icon:check[2x] |icon:x[2x]
|===
...
the link:/dev/reference#script-interpolation[Script interpolation] section
...
There are a number of commands that are interpolated in script, see
the link:/dev/reference#interpolated-miniscript-policy[Interpolated miniscript policy] section of the
